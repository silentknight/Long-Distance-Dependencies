...
class SequentialData(object):
  ...
  def add_data(self):
    ...
    self.averageLength = (self.averageLength * len(self.wordCountList) + len(self.__wordLine)) / (
              len(self.wordCountList) + 1)
...
class Corpus(object):
  ...
  def process_mobility(self, path):
    ...
    mobility_data = list(map(int, mobility_data))
    for trace in mobility_data:
      print(mobility_data[trace])
