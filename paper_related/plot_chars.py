...
with plt.style.context(('seaborn')):
    ...
    plt.loglog(np.arange(1,len(all_mi[3])+1), all_mi[3], label="WikiText2")
    # plt.loglog(np.arange(1,len(all_mi[0])+1), all_mi[0], label="Enwik8")
    # plt.loglog(np.arange(1,len(all_mi[4])+1), all_mi[4], label="WikiText103")
    ...
    plt.savefig('lm_chars', bbox_extra_artists=(lgd,), bbox_inches='tight')
    plt.show()
