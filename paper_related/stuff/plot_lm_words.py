...
filenames = ["wiki103_words_grassberger_logx_mi.dat","wiki2_words_grassberger_logx_mi.dat","penn_tree_words_grassberger_logx_mi.dat"]
# filenames = ["wiki103_words_standard_logx_mi.dat","wiki2_words_standard_logx_mi.dat","penn_tree_words_standard_logx_mi.dat","text8_words_standard_logx_mi.dat","enwik8_words_standard_logx_mi.dat"]
# filenames = ["wiki103_words_standard_log2_mi.dat","wiki2_words_standard_log2_mi.dat","penn_tree_words_standard_log2_mi.dat","text8_words_standard_log2_mi.dat","enwik8_words_standard_log2_mi.dat"]
...
#     lgd = ax.legend((p1, p2, p3, p4, p5), ("WikiText 103 words","WikiText 2 words","PennTree Banks words","Enwik8 words","Text8 words"), loc='upper right', shadow=True, fancybox=True) 
...
with plt.style.context(('seaborn')):
    ...
    # plt.loglog(np.arange(len(all_mi[3])), all_mi[3], label="Text8 words")
    # plt.loglog(np.arange(len(all_mi[4])), all_mi[4], label="Enwik8 words")
    plt.loglog(np.arange(1,len(all_mi[1])+1), all_mi[1], label="WikiText 2 words")
    plt.loglog(np.arange(1,len(all_mi[0])+1), all_mi[0], label="WikiText 103 words")
    plt.loglog(np.arange(1,len(all_mi[2])+1), all_mi[2], label="PennTree Banks words")
    ax = plt.axes()
